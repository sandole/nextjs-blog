...
1. **Cutting** — Extract the segment with precise timestamps
2. **Vertical conversion** — 9:16 aspect ratio with blurred background
3. **Caption burn-in** — ASS subtitles with pop-in animation
...
1. **Word-level timestamps are essential** — Chunk-level (~1s) creates awkward cuts. WhisperX's wav2vec2 alignment is worth the extra processing time.

2. **Keyword scoring is surprisingly effective** — Simple heuristics catch most funny moments. No need for complex sentiment analysis.

3. **Local LLMs are good enough** — llama3.2 (3B params) generates decent titles in ~10 seconds. No API costs.

4. **Profanity filtering is mandatory** — Both in titles and captions. YouTube's algorithm and advertisers don't like f-bombs in titles.

5. **Vertical blur background works** — The "podcast on phone" aesthetic consistently outperforms simple letterboxing.
...
**Curious about the results?** Check out the pipeline in action: [youtube.com/@funnypodcastcontents](https://youtube.com/@funnypodcastcontents?si=pDCW_8QwGg7XIUUy)
