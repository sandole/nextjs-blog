...
| sort by Count desc 
 
File Uplodaed Activity Over 2 Week Period 
 
OfficeActivity 
| where TimeGenerated > ago(14d) 
...
| render timechart 
 
FileUploaded by users Greater than 100 Over 2 Week Period  
 
OfficeActivity 
| where TimeGenerated > ago(14d) 
...
| where Identity <>"" 
// Domain Defence O365 
| where UserDisplayName matches regex "@.*@Defence O365" 
// Break down all the O365 by group 
| extend get_Group=extract("(@.*@Defence O365)", 1, UserDisplayName) 
//Get only the Group 
| extend Group=extract("@(.*)@Defence O365", 1,get_Group) 
| where isnotempty(Group) 
| project Group, Location, Identity  
| summarize Count=count() by Identity, Group, Location 
| order by Count desc 
| render columnchart 
 
