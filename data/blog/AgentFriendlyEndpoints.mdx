...
summary: How I added an RFC 9457-compliant agent digest endpoint and llms.txt to my Next.js blog, cutting token costs by 98% for AI agents crawling my site.
---
...
There's a simpler, complementary approach gaining traction: [llms.txt](https://llmstxt.org/). The idea is dead simple — put a Markdown file at `/llms.txt` that gives LLMs a concise overview of your site with links to detailed pages.

The format is structured enough for programmatic parsing but readable enough for LLMs to consume directly:
...
- [Building Agent-Friendly Endpoints](https://johnseong.ca/blog/AgentFriendlyEndpoints): RFC 9457 agent digest endpoint for Next.js
- [AI Debate Agents with A2A Protocol](https://johnseong.ca/blog/A2AProtocol): Multi-agent debate system using local LLMs
- [K9s for Kubernetes](https://johnseong.ca/blog/AKS): Terminal-based Kubernetes management
```

The spec also proposes `/llms-full.txt` — an expanded version where all the linked content is inlined into a single file. Instead of following links, an agent gets everything in one request. For a blog like this, `llms.txt` is the table of contents and `llms-full.txt` is the entire book.

The key difference from `/api/digest`: llms.txt is a static convention (like `robots.txt`), not a dynamic API. No content negotiation, no query parameters, no filtering. It's the lowest common denominator — any agent can fetch a well-known URL and get useful context without understanding your API contract.

They work well together. An agent discovers your site via `llms.txt` for a quick overview, then hits `/api/digest?tag=ai&limit=5` for targeted queries. Static discovery + dynamic filtering.
...
*The endpoint is live at [johnseong.ca/api/digest](https://johnseong.ca/api/digest). Agent discovery at [/.well-known/agent.json](https://johnseong.ca/.well-known/agent.json).*
